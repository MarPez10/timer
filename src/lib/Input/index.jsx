...
export const Input = ({ name, placeholder, type, register, errorText, label, style, errorTextHome }) => {
  const className = classNames(
    {
      [styles.scoreboard]: style === 'scoreboard',
      [styles.input]: style === 'input'
    }
  )
  const [isTypeText, setIsTypeText] = useState(false)
  const toggleType = () => setIsTypeText(prev => !prev)
  const errorAuth = (<div className={styles.errorAuth}>{errorText}</div>)
  const errorHome = (<div className={styles.errorHome}>{errorTextHome}</div>)
